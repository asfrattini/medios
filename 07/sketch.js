    ...
    function draw() {
      ...
      for (var gl = 0; gl < globos.length; gl++) {
        ...
        if (globos[gl].x >= 50 && globos[gl].x <= 200) {
          globos[gl].engordarse()
        }
        if (globos[gl].x > 200 && globos[gl].x <= 400) {
          globos[gl].enflaquecer()
        }
        if (globos[gl].x > 400 && globos[gl].x <= 500) {
          globos[gl].engordarse()
        }
        if (globos[gl].x > 500 && globos[gl].x <= 700) {
          globos[gl].enflaquecer()
        }
      }
    ...
    function nemo(miX, miY, miTipo) {
      ...
      this.dibujarse = function() {
        ...
        if (this.estaVivo == true) {
          if (this.tipo == 1) {
            image(nemo1, this.x, this.y)
          } else if (this.tipo == 2) {
            image(nemo2, this.x, this.y)
          }
        }
      }
